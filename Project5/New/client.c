...
int recvMsg(int fd){
    ...
    int out;
    out = open("output.txt",O_CREAT|O_APPEND|O_RDWR);
    while (recvfrom(fd,&buffer,sizeof(buffer),0,(struct sockaddr *)&receiver,&size) > 0){
        ...
        if (buffer.id == part){
            part++;
            write(out,&buffer.msg,sizeof(buffer));
        }else if(buffer.id > part){
            buffer.id = -1;
            if (sendto(fd,&buffer.id,sizeof(int),0,(const struct sockaddr *) &receiver, sizeof(receiver)) < 0){
...
struct sockaddr_in MEInfo(struct sockaddr_in server,int port){
